...
public:
	...
	__host__ __device__ m3Matrix(const m3Vector &col0, const m3Vector &col1, const m3Vector &col2) {
		r00 = col0.x; r01 = col1.x; r02 = col2.x;
		r10 = col0.y; r11 = col1.y; r12 = col2.y;
		r20 = col0.z; r21 = col1.z; r22 = col2.z;
	}

	__host__ __device__ m3Matrix(	m3Real c00, m3Real c01, m3Real c02,
	...
	__host__ __device__ void setColumns(const m3Vector &col0, const m3Vector &col1, const m3Vector& col2)
	{
		r00 = col0.x; r01 = col1.x; r02 = col2.x;
		r10 = col0.y; r11 = col1.y; r12 = col2.y;
		r20 = col0.z; r21 = col1.z; r22 = col2.z;
	...
	__host__ __device__ void setColumn(int i, const m3Vector &col) 
	{
	...
	__host__ __device__ void setRows(const m3Vector &row0, const m3Vector &row1, const m3Vector &row2)
	{
	...
	__host__ __device__ void setRow(int i, const m3Vector &row)
	{
	...
	__host__ __device__ void getColumns(m3Vector &col0, m3Vector &col1, m3Vector& col2) const 
	{
	...
	__host__ __device__ void getColumn(int i, m3Vector &col) const 
	{
	...
	__host__ __device__ m3Vector getColumn(int i) const 
	{
		m3Vector r; 
		getColumn(i, r);
		return r;
	...
	__host__ __device__ void getRows(m3Vector &row0, m3Vector &row1, m3Vector& row2) const 
	{
	...
	__host__ __device__ void getRow(int i, m3Vector &row) const 
	{
	...
	__host__ __device__ m3Vector getRow(int i) const 
	{
		m3Vector r; 
		getRow(i, r);
		return r;
	...
	__host__ __device__ m3Vector multiply(const m3Vector &v) const
	{
		m3Vector res;
	...
	__host__ __device__ m3Vector multiplyTransposed(const m3Vector &v) const 
	{
		m3Vector res;
	...
	__host__ __device__ void multiply(const m3Matrix &left, const m3Matrix &right) 
	{
		m3Matrix res;
	...
	__host__ __device__ void multiplyTransposedLeft(const m3Matrix &left, const m3Matrix &right) 
	{
		m3Matrix res;
	...
	__host__ __device__ void multiplyTransposedRight(const m3Matrix &left, const m3Matrix &right)
	{
		m3Matrix res;
	...
	__host__ __device__ m3Real get2D_determinant(m3Real a00, m3Real a01, m3Real a10, m3Real a11)
	{
		return a00 * a11 - a01*a10;
	}

	__host__ __device__ m3Real determinant()
	{
		return r00*(r11*r22 - r21*r12) - r01*(r10*r22 - r20*r12) + r02*(r10*r21 - r11*r20);
	}

	__host__ __device__ bool invert() 
	{
	...
	__host__ __device__ bool setInverse(m3Matrix &m)
	{
		m = *this;
		return m.invert();
	}
	...
	__host__ __device__ void setTransposed(m3Matrix &m)
	{
		m = *this;
		m.transpose();
	}

	__host__ __device__ m3Real magnitudeSquared()
	{
	...
	__host__ __device__ m3Real magnitude() { return sqrt(magnitudeSquared()); }
	...
	__host__ __device__ static void jacobiRotate(m3Matrix &A, m3Matrix &R, int p, int q);
	__host__ __device__ static void eigenDecomposition(m3Matrix &A, m3Matrix &R);
	__host__ __device__ static void polarDecomposition(const m3Matrix &A, m3Matrix &R, m3Matrix &S);
};
